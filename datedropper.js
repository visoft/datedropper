...
(function($) {
    var methods = {
        init : function(options) {
            return $(this).each(function() {
                if ($(this).is('input') && $(this).attr('type') == "text") {
                    ...
                    _dd_input.on('click.datedropper', function() {

                        _dd_set();

                    });

                    _dd_input.on('focusin.datedropper focus.datedropper', function(e){
                      e.preventDefault();
                    })

                    $(window).on('resize.datedropper', function() {
        ...
        remove : function() {
            $(this).off('.datedropper');
            $(window).off('.datedropper');
            var
                _dd_id = $('.dd-w').length - 1,
                _dd = $('#dd-w-' + _dd_id);
            _dd.remove();
        }
    };

    $.fn.dateDropper = function(methodOrOptions) {
        if ( methods[methodOrOptions] ) {
            return methods[ methodOrOptions ].apply( this, Array.prototype.slice.call( arguments, 1 ));
        } else if ( typeof methodOrOptions === 'object' || ! methodOrOptions ) {
            return methods.init.apply( this, arguments ); // Default to "init"
        } else {
            $.error( 'Method ' +  methodOrOptions + ' does not exist on dateDropper' );
        }
    };
}(jQuery));